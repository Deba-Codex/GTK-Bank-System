...
/* Save a transaction to TRANSACTION_FILE */
...
/* Load and display all transactions for the current user */
...
/* Logout callback: Clear current user and switch back to the login window */
...
/* Change password callback: Display a dialog to change password */
void on_change_password(GtkWidget *widget, gpointer data) {
    BankWidgets *widgets = (BankWidgets *)data;
    
    GtkWidget *dialog = gtk_dialog_new_with_buttons("Change Password", GTK_WINDOW(widgets->main_window),
...
/* Setup the UI: create a login window and a main banking window */
void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_grid_attach(GTK_GRID(main_grid), change_pass_button, 0, 4, 2, 1);
    ...
    gtk_grid_attach(GTK_GRID(main_grid), logout_button, 0, 5, 2, 1);
